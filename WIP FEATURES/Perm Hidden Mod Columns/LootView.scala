...
class LootView(val league: League)(implicit val pc: PoeCacher) extends View {
  ...
  def FlaskColumFilter = {
    setFilter(ComputedItemProps.FlaskInstant.shortName, ComputedItemProps.FlaskInstant.setInitialHeaderFilterValue)
  }
...
//  }
  ...
  private def appendGrid() {
    ...
    columns.all.foreach(col => { //initialize all column default filter value
      if (col.headerFilterValue.nonEmpty) filters.addColFilter(LootFilterColumn.parse(col.headerFilterValue, col))
    })
  ...
  val showItemDetailOnKeyPress = window.localStorage.getItem("showItemDetailOnKeyPress") == "true"
