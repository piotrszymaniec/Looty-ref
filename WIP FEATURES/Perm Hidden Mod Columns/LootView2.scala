...
//  }
  ...
  private def appendGrid() {
    ...
    //TODO add filter value to col
    //works but sets all, and i want to set only when column.toggle() fired
    columns.all.foreach(col => { //initialize all column default filter value
      if ((col.headerFilterValue.nonEmpty)) //add filter as column gets visible
        filters.addColFilter( LootFilterColumn.parse(col.headerFilterValue, col))


    })
//    columns.all.foreach(col => { //initialize all column default filter value
//      if ((col.headerFilterValue.nonEmpty)) //add filter as column gets visible //WLASCIWIE TO TU JUZ LEZY ZE TO NASZA NIEWIDOCZNA KOLUMNA, BO ZADNA INNA NIE MA TEGO USTAWIENIA!
//        //filters._hiddenColumnToggleFilter( col, false)
//        filters._hiddenColumnToggleFilter( LootFilterColumn.parse(col.headerFilterValue,col)) //a tu sprobujemy jednak stworzych lootfilter w tym plkiku
//    })

//jeszcze raz
//
//    columns.all.filter(c=>c.hiddenColumn).foreach(c => {
//    if (c.visible)
//        //setFilter(col.id,"1") //to robie zeby mojej kolumnie wstawic "1" ktora zreszta jest w col.headerFilterValue
//        setFilter(c.id,c.headerFilterValue)
//      else
//        setFilter(c.id,"")
//    })
    ...
    //    for (col <- columns.all) {
//      val vis = col.visible
//      val hid = col.hiddenColumn
//      val id = col.id
//      val text = col.headerFilterValue
//      filters.hiddenColumnToggleFilter(id)
//    }

/*
  does not work

    columns.all.filter(col => col.hiddenColumn) //pick only columns for mods that columns shall remain hidden in grid
       //pick those that their corresponding mods were turned on in Columns Select
      .foreach(col => filters.hiddenColumnToggleFilter(col.id,col.visible)) //add their desired filter
*/
  ...
  val showItemDetailOnKeyPress = window.localStorage.getItem("SETTINGS_ITEM_DETAILS_ON_KEY_PRESS") == "true"
    grid.onMouseEnter.subscribe((e: js.Dynamic, args: js.Any) => {
      if (showItemDetailOnKeyPress) {
