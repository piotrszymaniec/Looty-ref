...
object ComputedItemProps {
  ...
  class ComputedItemProp[A](
    ...
    val setInitialHeaderFilterValue: String,
    //val setJS: String, //when we want to setFilter() for column with fixed value, for say, set only corrupted items,
  ...
  private var vis: Boolean = true
    ...
    private[ComputedItemProps] def ??=(d:String): Unit = {
  ...
  def nno( // filter is showing numbers lesser then given value
    ...
    shortName: String,
    width: Int = 43,
    setInitialHeaderFilterValue: String = "")(
    groups: String*)(
    f: ComputedItem => Double): ComputedItemProp[Double] = {
    val res = new ComputedItemProp[Double](
      shortName = shortName,
  ...
  //TODO change this function so ComputedItemProp
  def boo( // affixes of boolean type, like FlaskCurseImmunity
           // filter is showing 1 or 0 values accordingly
    fullName: String,
    shortName: String,
    width: Int = 0)
    (setInitialHeaderFilterValue: String = "")( //set grid cell conent
    groups: String*)(
    ...
    val res = new ComputedItemProp[Double](
      ...
      width = width,
      groups = groups.distinct.toVector,
      defaultNumFilter = Some(NumFilter(_ >= _)), //if someone enters 0, then not show >= would // setting to < does not worked at all when i entered 1 filter like dont worked
      getJs = (i) => if (f(i)) 1.0 else 0.0,
      setInitialHeaderFilterValue = setInitialHeaderFilterValue
      //setJS = setJS
    )
    add(res)
    res
  ...
  //tests
  //val custom = new ComputedItemProp[Boolean]("corrupted","corruptem items",0,FlaskMods,defaultNumFilter = Some(""),getJs = (i) => i.flask.instantRecovery ,isColumnValFixed = true)
...
//  CustomScore2 !?= "Score according to semi-strict high value rules (roughly tier 2 and higher)"
  ...
  val FlaskInstant                 = boo("FlaskInstant", "finstant")("1")(FlasksActions)(_.flask.instantRecovery)
